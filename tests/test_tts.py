...
@pytest.mark.parametrize(
    ("suffix_surface", "suffix_reading", "suffix_pos1"),
    [
        ("くん", "クン", "接尾辞"),
        ("君", "クン", "接尾辞"),
        ("さん", "サン", "接尾辞"),
...
def test_normalize_kana_first_token_partial_separator_after_honorific_suffix(
    suffix_surface: str, suffix_reading: str, suffix_pos1: str
) -> None:
    class DummyFeature:
        def __init__(
            self,
    ...
    class DummyTagger:
        def __call__(self, _text: str):
            return [
                DummyToken("Rom", "ロム"),
                DummyToken(suffix_surface, suffix_reading, pos1=suffix_pos1, pos2="名詞的"),
                DummyToken("胸", "ムネ"),
            ]

    out = tts_util._normalize_kana_with_tagger(
        f"Rom{suffix_surface}胸",
        DummyTagger(),
        kana_style="partial",
        zh_lexicon=set(),
        force_first_token_to_kana=True,
    )
    assert out == f"ロム{suffix_surface}{_default_first_token_separator()}胸"


def test_normalize_kana_first_token_partial_separator_after_honorific_suffix_then_particle() -> None:
    class DummyFeature:
        def __init__(
            self,
            kana: str | None,
    ...
    class DummyTagger:
        def __call__(self, _text: str):
            return [
                DummyToken("Rom", "ロム"),
                DummyToken("さん", "サン", pos1="接尾辞", pos2="名詞的"),
                DummyToken("は", "ハ", pos1="助詞", pos2="係助詞"),
                DummyToken("胸", "ムネ"),
            ]
    ...
    assert out == f"ロムさんは{_default_first_token_separator()}胸"
...
def test_normalize_kana_separator_before_honorific_suffix_in_overrides_path() -> None:
    class DummyFeature:
        def __init__(
            self,
            kana: str | None,
    ...
    assert out == f"あおとさん{sep}胸"


def test_normalize_kana_separator_before_honorific_suffix_then_particle_in_overrides_path() -> None:
    class DummyFeature:
        def __init__(
            self,
            kana: str | None,
...
def test_normalize_kana_hiragana_particle_pronunciation_skips_mid_sentence_prepend() -> None:
    class DummyFeature:
        def __init__(
            self,
            kana: str | None,
    ...
    assert out == f"というわ{sep}むね{sep}"


def test_normalize_kana_hiragana_particle_pronunciation_keeps_noha_attached() -> None:
    class DummyFeature:
        def __init__(
            self,
            kana: str | None,
    ...
    class DummyTagger:
        def __call__(self, _text: str):
            return [
                DummyToken("殺し", "コロシ"),
                DummyToken("た", "タ"),
                DummyToken("の", "ノ", pos1="助詞", pos2="連体化"),
                DummyToken("は", "ハ", pos1="助詞", pos2="係助詞"),
                DummyToken("胸", "ムネ"),
            ]
    ...
    assert out == f"ころし{sep}たのは{sep}むね{sep}"
    assert "のわ" not in out


@pytest.mark.parametrize(
    "lead_surface",
    ["に", "で", "と", "が", "も", "や", "か", "へ", "を", "から", "まで", "より", "だけ"],
)
def test_normalize_kana_hiragana_particle_pronunciation_keeps_ha_after_particles(
    lead_surface: str,
) -> None:
    class DummyFeature:
        def __init__(
    ...
    class DummyTagger:
        def __call__(self, _text: str):
            return [
                DummyToken("話し", "ハナシ"),
                DummyToken(lead_surface, lead_surface, pos1="助詞", pos2="格助詞"),
                DummyToken("は", "ハ", pos1="助詞", pos2="係助詞"),
                DummyToken("胸", "ムネ"),
            ]
    ...
    sep = _default_first_token_separator()
    out = tts_util._normalize_kana_with_tagger(
        f"話し{lead_surface}は胸",
        DummyTagger(),
        kana_style="hiragana",
    )
    assert out == f"はなし{sep}{lead_surface}は{sep}むね{sep}"
    assert f"{lead_surface}わ" not in out
...
def test_normalize_kana_text_rebalances_existing_separator_without_kanji(
    ...
    sep = _default_first_token_separator()
    text = f"「{sep}あおと{sep}くんだからじゃないよ。{sep}あおと{sep}くん」"

    class DummyTagger:
        def __call__(self, got: str):
            assert got == text
            return [
                DummyToken("「"),
                DummyToken(sep),
    ...
    assert out == f"「あおとくん{sep}だからじゃないよ。あおとくん{sep}」"
